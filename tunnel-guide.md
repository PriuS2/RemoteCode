...
| 방식 | 스크립트 | URL | 용도 |
|------|----------|-----|------|
| Named Tunnel | `tunnel.ps1` | `https://pri-u.net` | 고정 도메인, 운영/상시 사용 |
...
CCR_DOMAIN=pri-u.net   # Cloudflare Tunnel 도메인
```
...
```
pri-u.net → Cloudflare Tunnel → localhost:{CCR_VITE_PORT} (Vite)
...
```powershell
cloudflared tunnel route dns ccr-tunnel api.pri-u.net
```
...
# https://dash.cloudflare.com → pri-u.net → DNS
```
